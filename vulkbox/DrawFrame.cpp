...
void HelloTriangleApplication::updateUniformBufferWithPhysics(uint32_t currentImage, float elapsed)
{
    ...
    physx::PxControllerCollisionFlags cflags = this->mController->move(
            ...
            if (bx >= 0 && bx < x_extent) {
                for (int by = iy - 1; by < iy + 2; by++) {
                    if (by >= 0 && by < y_extent) {
                        if (!(bx == ix && by == iy)) {
                            if ((iz + 1) < z_extent) {
                                idx1 = GRIDIDX(bx, by, iz + 1);
                            }
                            else {
                                idx1 = 0;
                            }
                            if (this->pBlockArray[GRIDIDX(bx, by, iz)] == 1 || this->pBlockArray[idx1] == 1)
                            {
...
cant_go_there:
    ...
    if (ix >= 0 && ix < x_extent)
    {
        if (iy >= 0 && iy < y_extent)
        {
            if (iz >= 0 && iz < z_extent)
            {
                if (this->pBlockArray[GRIDIDX(ix, iy, iz - 1)] == 1) {
                    floor = (float)iz;
                }
            }
        }
